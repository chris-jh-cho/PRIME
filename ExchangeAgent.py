...
class ExchangeAgent(FinancialAgent):
  ...
  def receiveMessage(self, currentTime, msg):
    ...
    if msg.body['msg'] == "WHEN_MKT_OPEN":
    ...
    elif msg.body['msg'] == "QUERY_ORDER_SIGN":
      symbol = msg.body['symbol']
      length = msg.body['length']

      if symbol not in self.order_books:
        log_print("Past trade sign request discarded.  Unknown symbol: {}", symbol)
      else:
        log_print("{} received QUERY_ORDER_SIGN ({}:{}) request from agent {}", self.name, symbol, length,
                  msg.body['sender'])

      # 20200307 Chris Cho - The order_books.history is in a bad form. The below function restructures it.

      def get_order_sign(history, length):
        ...
        for i in range(len(a)):
          if a.transactions[i] != []:
            dummy.append(a.is_buy_order[i])

        sign_list = dummy[:length]

        return sign_list

      sign_list = get_order_sign(self.order_books[symbol].history, length)
      ...
      # rather than exclusing it - note that this returns everything upto the last "trade" i.e. a successful
      # transaction.
      self.sendMessage(msg.body['sender'], Message({"msg": "QUERY_ORDER_SIGN", "symbol": symbol, "length": length,
                                                    "mkt_closed": True if currentTime > self.mkt_close else False,
                                                    "signs": sign_list#self.order_books[symbol].history[0:(length + 1)]
                                                    }))
  ...
  def logOrderBookSnapshots(self, symbol):
    ...
    if book.book_log:
      ...
      if not self.wide_book:
        ...
        #df = pd.DataFrame(pd.arrays.SparseArray(index=dfLog.index))
